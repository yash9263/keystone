...
import { Alert, BlankState, Center, Spinner, GlyphButton, ResponsiveText } from '../../../../elemental';
...
const RelatedItemsList = React.createClass({
  ...
  getCreateValues() {
    var ret = {}
    if (this.props.data) {
      if (this.props.data.fields.company) {
        Object.assign(ret, {company: this.props.data.fields.company})
      }
      Object.assign(ret, {product: this.props.data.id})
    }
    return ret
  },
  render () {
    ...
    return (
      <div className="Relationship">
        <CreateForm
          ...
          values={this.getCreateValues()}
        />
        <h3 className="Relationship__link"><Link to={listHref}>{this.props.refList.label}</Link></h3>
        <GlyphButton data-e2e-item-create-button="true" color="success" glyph="plus" position="left" onClick={this.showCreateForm}>
          <ResponsiveText hiddenXS="Create" visibleXS="Create" />
        </GlyphButton>
